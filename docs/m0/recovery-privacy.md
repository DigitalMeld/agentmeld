...
The explicit cancellation probe starts a child and grandchild that ignore SIGTERM and append synthetic heartbeat records to their disposable workspace. The host persists cancellation, stops only the uniquely named owned container with a one-second grace period, and verifies its immutable ID is absent from a successful Docker inventory. It also checks that heartbeat bytes remain unchanged and that supervisor replacement retains cancelled state.
...
This qualifies whole-container termination on the dedicated local Linux ARM64 runtime, including uncooperative descendants. It is not yet wired into every viewer/native cancellation path, does not undo external effects, and does not qualify remote runtime outages or stopping individual tasks inside a shared container.
...
Verified: 48 default tests (13 Rust, 33 Node, 2 Python), formatting, Clippy, build and documentation checks passed. All four container probes passed. The hidden viewer rendering was visually inspected. Cancellation completed in 285 ms in one local sample, including the 200 ms post-stop heartbeat observation. The tested image is `sha256:391a6baf59845c75045e7115dd230b9001e5c429f1fb47e0b7b871a7e830d898`; it is not published. Reports, journals and screenshots remain under ignored `.local/m0/`. Next M0 work remains authenticated worker identity, broader native tools, scoped provider credentials/egress, live inference, workspace quotas and designated iMessage setup.
