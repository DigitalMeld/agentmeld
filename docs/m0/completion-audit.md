...
| --- | --- | --- |
| Pinned runtime, subscription authentication, licenses | Codex 0.154.0, pinned base images and [installed inventory](distribution-inventory.md); owner-authorized narrow native auth import, managed device challenge/cancel and live subscription pass in [auth report](codex-subscription.md) | Shared owner refresh/logout lifecycle is not qualified. Do not revoke the imported owner session to test it. Embedded native notices and source-license selection remain release obligations. |
...
| Browser, viewer, takeover, fresh resume | Sandboxed Chromium and [viewer fixture](viewer-takeover.md) pass with Rust generation fencing, private-screen suppression and fresh observation before dispatch resumes. Playwright remains the pinned browser component; narrow screenshot HTTP transport is local experimental code. | Integrate this controller with the authenticated native tool path. No final production video/remote-viewer component is selected; keyboard, clipboard and owner credential entry remain unqualified. |
| Initial resource measurements | [150-second offline baseline](resource-baseline.md): 51 native/browser cycles, zero OOM kills, peak charged memory 212.04 MiB. [64 MiB workspace](workspace-disk-limit.md) proves disk-full recovery and authenticated replacement-worker continuation. | Tested envelope is a 2-vCPU/4-GiB dedicated VM and bounded workers, not a published minimum-host recommendation. Live workload sizing, longer soaks and whole-VM recovery remain unqualified. |
...
| Conversation after process/worker replacement | Live GPT-5.5, protected native history and bounded persistent workspace | Pass; not whole-machine recovery |
| Browser takeover and fresh resume | Offline real Chromium, Rust authority, synthetic page | Pass separately; authenticated integration open |
...
1. Connect the real subscription-backed native tool callback to the already-qualified separate browser and Rust controller. Exercise takeover, stale dispatch denial and fresh-observation resume through that integrated path.
...
3. Qualify remaining session/recovery cases with dedicated fixtures that preserve the owner session. Record viewer selection and the supported test envelope before M0 sign-off.
