...
Returned content remains untrusted tool data. A matching hash establishes internal consistency, not honesty of a compromised worker. Configured native-probe results are now saved by the [Rust result archive](result-archive.md) before settlement. Journal format 6 now indexes the receipt in the settlement record. The archive and journal remain separate writes, and there is no production artifact API. Descriptor checks detect ordinary concurrent changes but do not provide an atomic snapshot against a malicious writer. Parent directories and the mount are trusted; this is not a general host filesystem sandbox. General native built-in tools remain outside this adapter.

Next: authenticated result retrieval and explicit reconciliation, broader native-tool policy, provider credentials/egress and actual model qualification. Do not advertise arbitrary file access, immutable snapshots or production artifact recovery from this fixture.
