...
Current local availability: after regression-build cleanup and VM restart, Docker lists no runtime images. The credential volume remains present. Restore the baseline/candidate and verify restart retention before further qualification; see [storage readback](build-storage.md#regression-build-cleanup-readback).
