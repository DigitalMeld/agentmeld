...
1. **Multi-host streams.** The alpha has multiple Mac hosts and explicit multi-host selection. Should a device open one SSE stream per selected host (this design's assumption — host-scoped streams, host-scoped cursors), or should the service offer a multiplexed stream across all hosts the device is enrolled on? Per-host is simpler and matches the host-scoped journal; multiplexed is friendlier for the iOS "away-from-home" surface but needs a cross-host cursor story this design deliberately avoids.
2. **`delivery_outbox` timing.** The unification design's Phase 4 bullet mentions the notification-intent outbox as "push-ready"; this design keeps `delivery_outbox` out of Phase 4 (SSE is the live path; durable push intents stay a separate increment). Confirm the exclusion, or pull the outbox table installation into Phase 4 so a future APNs sender has something to drain?
3. **Idle-host lease visibility.** A lease transition with no affected run (e.g., takeover of an idle host) journals no stream row in this design — clients learn it via `GET /api/v1/lease`. Acceptable, or should the stream carry host-level lease events with `run_id: null` so the future Computer surface can subscribe to takeovers without polling?
