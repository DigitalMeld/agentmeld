...
- [Repeatable headless-browser E2E 2026-09-19](progress/2026-09-19-e2e-browser-script.md): `scripts/e2e_browser.py` (issue #99) — seed → serve → pair → drive → assert in one command against a disposable loopback server; 16/16 assertions green, including the seed-after-boot decision forced by the `service_restart` startup recovery.
- [Browser lease controls 2026-09-19](progress/2026-09-19-lease-controls.md): take/release/private-session affordances around the lease pill (issue #101), client-only against existing endpoints — two-step takeover, generation-fenced mutations, heartbeat while holding; E2E extended to 20/20.
- [First-run pairing UI 2026-09-19](progress/2026-09-19-pairing-ui.md): bare URL shows a pairing screen instead of a false "session expired" (issue #103); expired sessions get "Pair a new device"; E2E now pairs through the UI, 22/22.
- [User-initiated approval revocation 2026-09-19](progress/2026-09-19-approval-revoke.md): `POST /api/v1/approvals/{id}/revoke` (issue #105) — pulling back a grant kills the worker's execution ticket; history UI gets a two-step Revoke button with honest already-dispatched copy; migration v6 extends the revoked_reason enum; E2E 26/26.
- [Device management 2026-09-19](progress/2026-09-19-device-mgmt.md): `GET /api/v1/devices` (issue #107) — the "This Mac" host block opens a Devices dialog listing every paired device with session counts and a two-step Revoke button wired to the existing kill switch; E2E 33/33.
- [Hide the dead Schedule tab 2026-09-19](progress/2026-09-19-hide-schedule.md): the inspector's Schedule tab was a dead placeholder with no backend — removed for the alpha (issue #109); tab keyboard navigation verified intact.
- [Stream outage UX 2026-09-19](progress/2026-09-19-stream-outage-ux.md): `Reconnecting… · updated Xm ago` plus an explicit Retry stream button after sustained SSE failure; also fixed the host-block handlers opening the Devices dialog when the retry button is clicked.
