...
The headless Chromium fixture now passes with the documented browser seccomp policy. Next: qualify viewer takeover, scoped authenticated provider execution and a real local Ollama model. License selection and a designated iMessage test identity remain open. M0 has not exited; M1 UI work has not begun.
