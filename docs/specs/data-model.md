...
The [workspace browser](../design/workspace-browser.md) adds optional `conversation.workspaceCapturedAt` and snapshot file `modifiedAt` fields. These timestamps are not synthesized for older records. Read-only workspace APIs project all retained entries while excluding provider session data. This remains additive JSON version-2 behavior, not a SQL migration.
