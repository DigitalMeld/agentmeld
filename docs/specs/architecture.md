...
An interrupted native turn is not proof that its commands stopped. The pinned Codex unified-exec contract can retain background terminals after interruption. On owner cancellation, revoke dispatch first, request native interruption and explicit terminal cleanup, then verify termination. If native cleanup cannot establish the required stop, use the run's owned-worker termination boundary and confirm it through the runtime; report termination as unconfirmed until readback succeeds. Keep this distinct from ordinary turn completion, which may intentionally preserve a background session. See [live control qualification](../m0/live-control.md#explicit-terminal-cleanup-follow-up).
