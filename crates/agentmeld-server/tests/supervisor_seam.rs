...
const STUB_WORKER_V2: &str = r#"// Stub worker-seam/2 worker: negotiates v2, emits tool events, completes.
import net from 'node:net';
import fs from 'node:fs';
import path from 'node:path';
import process from 'node:process';
...
(async () => {
  send({ protocol: 'worker-seam/2', msg_id: 'hello-1',
         msg_type: 'worker.session.hello', worker_pid: process.pid, worker_token: token });
  const welcome = await nextMsg();
  if (welcome.msg_type !== 'service.session.welcome' || welcome.ok !== true) process.exit(3);
  if (welcome.negotiated_protocol !== 'worker-seam/2') process.exit(10);
  const start = await nextMsg();
  if (start.msg_type !== 'service.turn.start') process.exit(4);
  if (start.protocol !== 'worker-seam/2') process.exit(11);
  fs.writeFileSync(path.join(start.turn.staging_dir, 'workspace-listing.json'), '[]');
  // v2 tool events: the worker calls a tool and finishes it.
  send({ protocol: 'worker-seam/2', msg_id: 'ev-t1', msg_type: 'worker.events.append',
         run_id: start.run_id, generation: start.generation,
         events: [{ event_id: 'ev-t1', event_type: 'tool.call_started',
                    payload: { call_key: 'probe-1', tool_name: 'computer.screenshot',
                               title: 'take a screenshot' } }] });
  const t1ack = await nextMsg();
  if (t1ack.msg_type !== 'service.events.stored') process.exit(5);
  send({ protocol: 'worker-seam/2', msg_id: 'ev-t2', msg_type: 'worker.events.append',
         run_id: start.run_id, generation: start.generation,
         events: [{ event_id: 'ev-t2', event_type: 'tool.call_finished',
                    payload: { call_key: 'probe-1', state: 'completed',
                               result_json: { bytes: 42 } } }] });
  const t2ack = await nextMsg();
  if (t2ack.msg_type !== 'service.events.stored') process.exit(12);
  send({ protocol: 'worker-seam/2', msg_id: 'ev-1', msg_type: 'worker.events.append',
         run_id: start.run_id, generation: start.generation,
         events: [{ event_id: 'ev-1', event_type: 'run.completed', payload: {} }] });
  const stored = await nextMsg();
  if (stored.msg_type !== 'service.events.stored') process.exit(13);
  send({ protocol: 'worker-seam/2', msg_id: 'ev-2', msg_type: 'worker.events.append',
         run_id: start.run_id, generation: start.generation,
         events: [{ event_id: 'ev-2', event_type: 'run.terminated',
                    payload: { cleanup_ok: true } }] });
  const termAck = await nextMsg();
...
fn setup_v2() -> (Arc<Db>, std::path::PathBuf, std::path::PathBuf) {
    let (db, dir, repo_root) = setup();
    std::fs::write(repo_root.join("apps/worker/worker.mjs"), STUB_WORKER_V2).unwrap();
    (db, dir, repo_root)
}

#[tokio::test]
async fn stub_worker_v2_negotiates_and_journals_tool_steps() {
    let (db, dir, repo_root) = setup_v2();
    let state_dir = dir.join("state");
    ...
    // The v2 tool events projected exactly one completed tool step.
    let steps = db.tool_steps_for_run(&run_id).expect("read steps");
    assert_eq!(steps.len(), 1);
    assert_eq!(steps[0].call_key, "probe-1");
    assert_eq!(steps[0].tool_name, "computer.screenshot");
    assert_eq!(steps[0].state, "completed");
    assert_eq!(steps[0].result_json.as_deref(), Some(r#"{"bytes":42}"#));
    common::cleanup(&dir);
}
