...
export async function runCommandFixture({ home, command, patch, imagePath, unifiedExec }) {
  ...
  const server = createServer((req, res) => {
    (async () => {
      ...
      imageReturned ||= JSON.stringify(body.input).includes('data:application/octet-stream') || JSON.stringify(body.input).includes('data:image/');
      ...
      const args = imagePath !== undefined ? { path: imagePath } : selectedTool === 'exec_command'
      ...
      const item = returned
        ...
        : patch !== undefined
          ? { type: 'custom_tool_call', id: 'fc_boundary', call_id: 'call_boundary', name: selectedTool, input: patch }
  ...
  const fail = () => { if (closing) return; fatal = new Error('native command fixture failed'); rejectDone(fatal); for (const waiter of pending.values()) waiter.reject(fatal); pending.clear(); proc.kill('SIGKILL'); };
  ...
  try {
    ...
    const started = await request('thread/start', { cwd: '/workspace', model: 'gpt-5.5', modelProvider: 'm0', approvalPolicy: 'on-request', ephemeral: true });
