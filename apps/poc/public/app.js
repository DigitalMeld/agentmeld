...
let detailId=null,lastDetail='',detailOrigin=null,detailContext='activity';
function detailTasks(){const task=state.tasks.find(t=>t.id===detailId);return detailContext==='conversation'?activityTasks(state,{conversationId:task?.conversationId,oldest:true}):currentActivity();}
$('runDetails').addEventListener('close',()=>{if(detailOrigin?.isConnected)detailOrigin.focus({preventScroll:true});});
...
$('runConversation').onclick=()=>{const task=state.tasks.find(t=>t.id===detailId);detailOrigin=null;$('runDetails').close();if(task)jumpToTask(task);};
function renderDetails(){
 ...
 // The Tool steps section only renders when steps are known; after a
 // reload the SSE cursor skips historical tool events, so backfill from
 // the persisted projection and re-render if steps appeared.
 if(track)void track.hydrateSteps(task.id).then(added=>{if(added&&detailId===task.id&&$('runDetails').open)renderDetails();});
...
function renderActivity(){
 ...
 const date=new Date(task.createdAt),key=date.toLocaleDateString(),heading=key!==day?'<h3>'+esc(dayLabel(date))+'</h3>':'';day=key;
 const title=state.conversations.find(c=>c.id===task.conversationId)?.title||'Conversation';
 return heading+'<article class="activityEntry"><button class="activityLink" data-jump="'+task.id+'" aria-label="Open conversation: '+esc(task.prompt)+'"><strong>'+esc(task.prompt)+'</strong><span class="activityMeta">'+'<span class="statusBadge" data-status="'+esc(task.status)+'">'+esc(statusLabel(task.status))+'</span> · '+esc(date.toLocaleTimeString([], {hour:'numeric',minute:'2-digit'}))+(duration(task)!==null?' · '+elapsedLabel(duration(task)):'')+'</span>'+(title!==task.prompt?'<span class="activityChat">'+esc(title)+'</span>':'')+((task.error||!['Finished','Completed','Done',statusLabel(task.status)].includes(task.activity))?'<span class="activitySummary">'+esc(task.error||task.activity)+'</span>':'')+(task.inputs.length||task.artifacts.length?'<span class="activityFileCount">'+countLabel(task.inputs.length,'input')+' · '+countLabel(task.artifacts.length,'output')+'</span>':'')+'</button><button class="detailLink" data-detail="'+task.id+'">View details</button>'+task.artifacts.map(f=>fileButton(task,f)).join('')+'</article>';
...
function workspaceMarkup(query){
  ...
  const entries=!workspaceLoading&&workspace?.conversationId===workspaceId?browseWorkspace(workspace.entries,workspacePath,query,$('showHiddenFiles').checked,workspaceSort,workspaceDescending):[];workspaceEntries=entries;
...
async function openWorkspaceFile(name,id=workspaceId){
  const request=++previewRequest;previewTarget={conversationId:id,name};beginPreview(name);
  try{const blob=await(await api('/api/workspace/file?conversation='+encodeURIComponent(id)+'&name='+encodeURIComponent(name),{signal:AbortSignal.any([previewController.signal,AbortSignal.timeout(8000)])})).blob();
  ...
  }catch(e){if(request===previewRequest)failPreview('Could not open this workspace file. Retry to try again.');}
}

function setPreviewBusy(busy){$('preview').setAttribute('aria-busy',String(busy));for(const id of ['downloadFile','copyFilePath','copyFilename','copyPreview','previewConversation','findInPreview','wrapPreview','previewSource','previewVersion','previousFile','nextFile'])$(id).disabled=busy;if(!busy){updatePreviewNavigation();const binary=previewFile?.text===undefined;$('findInPreview').disabled=$('wrapPreview').disabled=binary;}}
function beginPreview(name){previewController?.abort();previewController=new AbortController();if(!$('preview').open)previewOriginFocus=document.activeElement;$('previewLoadError').hidden=true;$('retryPreview').disabled=true;previewFocus=$('preview').contains(document.activeElement)?document.activeElement:null;clearPreviewImage();previewReader.reset();previewFile=null;$('previewTitle').textContent=name;$('previewTitle').title=name;$('previewOrigin').textContent='';$('previewMetadata').textContent='Loading…';$('previewPosition').textContent='';$('previewWarning').hidden=true;$('previewBody').textContent='Loading file…';$('previewBody').scrollTop=0;$('versionLabel').hidden=true;$('previewSource').hidden=true;setPreviewBusy(true);if(!$('preview').open)$('preview').showModal();}
function failPreview(message){$('preview').setAttribute('aria-busy','false');$('previewMetadata').textContent='';$('previewBody').textContent='';$('previewLoadMessage').textContent=message;$('previewLoadError').hidden=false;$('retryPreview').disabled=false;}
$('retryPreview').onclick=()=>{const target=previewTarget;if(!target)return;if(target.conversationId)openWorkspaceFile(target.name,target.conversationId);else openPreview(target.taskId,target.name);};
...
let previewImageUrl=null;function clearPreviewImage(){for(const img of $('previewBody').querySelectorAll('img')){img.onload=null;img.onerror=null;}for(const media of $('previewBody').querySelectorAll('audio,video')){media.onloadedmetadata=null;media.onerror=null;media.pause();media.removeAttribute('src');media.load();}if(previewImageUrl)URL.revokeObjectURL(previewImageUrl);previewImageUrl=null;}
$('preview').addEventListener('close',()=>{previewRequest++;previewController?.abort();previewController=null;previewTarget=null;previewFile=null;previewReader.reset();clearPreviewImage();$('previewBody').replaceChildren();if(previewOriginFocus?.isConnected)previewOriginFocus.focus({preventScroll:true});previewOriginFocus=null;$('preview').classList.remove('expanded');$('expandPreview').setAttribute('aria-pressed','false');$('expandPreview').textContent='Expand';});
...
function renderPreviewContent(){
 ...
 if(types[ext]){const type=types[ext],tag=type.startsWith('image')?'img':type.startsWith('audio')?'audio':'video';previewImageUrl=URL.createObjectURL(new Blob([blob],{type}));const el=document.createElement(tag);el.src=previewImageUrl;el.className='fileMedia';el.setAttribute('aria-label',name);if(tag==='img'){el.alt=name;el.classList.add('fileImage');el.onload=()=>{if(!el.isConnected)return;$('previewMetadata').textContent+=' · '+el.naturalWidth+' × '+el.naturalHeight;};}else{el.controls=true;el.preload='metadata';el.onloadedmetadata=()=>{if(el.isConnected&&Number.isFinite(el.duration))$('previewMetadata').textContent+=' · '+elapsedLabel(el.duration*1000);};}el.onerror=()=>{if(!el.isConnected)return;const msg=document.createElement('p');msg.textContent='This format could not be previewed. You can still download the original file.';$('previewBody').replaceChildren(msg);};$('previewBody').replaceChildren(el);return;}
...
$('previewConversation').onclick=()=>{previewOriginFocus=null;if(previewFile?.conversationId){$('preview').close();selectConversation(previewFile.conversationId);return;}const task=state.tasks.find(t=>t.id===previewFile?.taskId);$('preview').close();if(task)jumpToTask(task);};
...
async function openPreview(taskId,name){
  const request=++previewRequest;previewTarget={taskId,name};beginPreview(name);
  try{
    const res=await api('/api/file?task='+encodeURIComponent(taskId)+'&name='+encodeURIComponent(name),{signal:AbortSignal.any([previewController.signal,AbortSignal.timeout(8000)])});const blob=await res.blob();
  ...
  }catch(e){if(request===previewRequest)failPreview('Could not load this file. Retry to try again.');}
}
...
document.addEventListener('click',async e=>{
  ...
  const detail=e.target.closest('[data-detail]');if(detail){detailOrigin=detail;detailContext=detail.closest('#conversation')?'conversation':'activity';detailId=detail.dataset.detail;lastDetail='';renderDetails();$('runBody').scrollTop=0;$('runDetails').showModal();}
...
function moveRun(delta){const list=detailTasks(),index=list.findIndex(t=>t.id===detailId);if(index>=0&&list[index+delta]){detailId=list[index+delta].id;renderDetails();$('runBody').scrollTop=0;if(document.activeElement===document.body)$('runBody').focus({preventScroll:true});}}
