...
let state={tasks:[],active:null},selected=null,files=[],view='chat',lastRender='';
...
async function api(path,options={}){const res=await fetch(path,{...options,headers:{Authorization:'Bearer '+token,'Content-Type':'application/json',...options.headers}});if(!res.ok){const e=await res.json();throw Error(e.error);}return res;}
function error(text=''){$('error').textContent=text;}
function showChat(){view='chat';render();}
function newTask(){selected=null;$('prompt').value='';files=[];error();showChat();$('prompt').focus();}
$('newChat').onclick=newTask;$('mobileNew').onclick=newTask;
$('chatNav').onclick=showChat;$('filesNav').onclick=()=>{view='files';render();};
...
function render(){
  ...
  $('history').innerHTML=state.tasks.length?[...state.tasks].reverse().map(t=>'<button class="historyItem '+(t.id===selected?'selected':'')+'" data-select="'+t.id+'">'+esc(t.prompt)+'</button>').join(''):'<div class="emptyHistory">Your tasks will appear here.</div>';
  const task=state.tasks.find(t=>t.id===selected);
  const content=task?'<div class="time">'+esc(new Date(task.createdAt).toLocaleString([], {month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}))+'</div><div class="turn"><div class="message user">'+esc(task.prompt)+(task.inputs.length?'<div class="messageLabel">'+task.inputs.map(f=>esc(f.name)).join(' · ')+'</div>':'')+'</div>'+(task.answer?'<div class="message assistant">'+formatted(task.answer)+'</div>':'')+(task.status==='running'?'<div class="pending"><span class="pulse"></span>'+esc(task.activity)+'</div>':'<div class="pending">'+esc(task.error||task.activity)+'</div>')+task.artifacts.map(f=>fileButton(task,f)).join('')+'</div>':'<div class="welcome"><span class="avatar large"><img src="/brain.svg" alt="" aria-hidden="true"></span><h1>What would you like to get done?</h1><p>Bring a file and a question.<br>I’ll do the work and bring back the result.</p><button class="suggestion" id="sample">Find the story in my sales data<small>Try a sample CSV and get a real report ↗</small></button></div>';
  ...
  $('send').hidden=!!state.active;$('stop').hidden=!state.active;$('prompt').disabled=!!state.active;
...
document.addEventListener('click',async e=>{
  const choose=e.target.closest('[data-select]');if(choose){selected=choose.dataset.select;showChat();}
  const remove=e.target.closest('[data-remove]');if(remove){files.splice(Number(remove.dataset.remove),1);render();}
...
$('upload').onchange=async()=>{
  try{for(const f of $('upload').files){if(f.size>2*1024*1024)throw Error('Attach files smaller than 2 MB.');if(files.length>=5)throw Error('Attach up to five files.');const bytes=new Uint8Array(await f.arrayBuffer());let binary='';for(let i=0;i<bytes.length;i+=8192)binary+=String.fromCharCode(...bytes.subarray(i,i+8192));files.push({name:f.name,data:btoa(binary)});}render();}catch(e){error(e.message);}finally{$('upload').value='';}
};
$('composer').onsubmit=async e=>{
  e.preventDefault();error();const prompt=$('prompt').value.trim();if(!prompt||state.active)return;
  try{const res=await api('/api/tasks',{method:'POST',body:JSON.stringify({prompt,files})});const task=await res.json();selected=task.id;files=[];$('prompt').value='';await refresh();}catch(e){error(e.message);}
};
$('prompt').onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();$('composer').requestSubmit();}};
$('stop').onclick=async()=>{try{$('stop').disabled=true;await api('/api/stop',{method:'POST',body:'{}'});await refresh();}catch(e){error(e.message);}finally{$('stop').disabled=false;}};
async function refresh(){try{state=await(await api('/api/state')).json();if(!selected&&state.active)selected=state.active;render();}catch(e){error(e.message);}}
