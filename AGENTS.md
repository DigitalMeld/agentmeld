...
- Working cadence is issues + PRs: open a GitHub issue for the work, work on a branch, submit a PR for review. Never push directly to main. Write issues and PRs with full context (the decision, alternatives considered, verification state, open questions) so a different agent can pick up the work cold.
